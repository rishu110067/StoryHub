...
router.post('/', ensureAuth, async (req, res) => {
    try {
        ...
        req.body.length = req.body.body.length;
...
router.get('/:id', ensureAuth, async (req, res) => {
    try {
        ...
        let favStory = await FavStory.findOne({story: req.params.id, user: req.user.id}).lean();
        let isFav = false;
        if(favStory) {
            isFav = true;
        }
    ...
    try {
        ...
        if(story.user != req.user.id) {
            ...
            if(req.body.status == "private") {
                await FavStory.remove({story: req.params.id});
            }
            res.redirect('/dashboard');
        }
    } catch (err) {
